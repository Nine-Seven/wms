        ...
        if(this.grid.fireEvent('beforeheaderfiltersapply', this.grid, filters, this.grid.getStore()) !== false)
        {
            ...
            for(var fn in filters)
            {
                ...
                else
                {
                    ...
                    {
                    	var filterValues = filters[fn];
                    	if(field.fieldLabel == '商品大类'){
                    		filterValues = proTypeArray;//proTypeArray页面变量值
                    		//alert(filterValues);
                    	}
                        var newSf = new Ext.util.Filter({
                            root: this.filterRoot,
                            property: fn,
                            value: eval("/"+filterValues+"/"),
                            label: field.fieldLabel
                        });
                        if(sf)
                        {
            ...
            if(change)
            {
                ...
                this.grid.invalidateScrollerOnRefresh= false;
            }
        }
    },

...
var gridOutstockM3504store=Ext.create('cms.store.odata.odata_OutstockMStore',{
	proxy:{
		type:'ajax',
		method:'post',
		...
		if(th.count()>0){
			Ext.getCmp('gridOutstockM3504').getSelectionModel().select(0);
		}
	  }
	}
});
var gridOutstockD3504store=Ext.create('cms.store.odata.odata_OutstockDStore',{
	proxy:{
		type:'ajax',
		method:'post',
...
var gridOutstockDLabel3504store=Ext.create('cms.store.odata.odata_OutstockDStore',{
	proxy:{
		type:'ajax',
		method:'post',
...
Ext.define('cms.view.odata.odata_B2COutstockLabelWaterUI',{
	alias:'widget.odata_B2COutstockLabelWaterUI',
	title:'电商流水标签回单',//电商流水标签回单
	width:'100%',
	layout:'border',
	    	    ...
	    	    items:[
	    				{
	    					...
	    					store:Ext.create("cms.store.common.comboStore",
	    					{
	    						...
	    						listeners:{  
		    						'load':function(th,records,successful,eOpts ){
		    							if(th.count()>0){
		    								Ext.getCmp('cmbWave_no3504').setValue(th.getAt(0).data.value);
		    								_myAppGlobal.getController('cms.controller.odata.odata_B2COutstockLabelWaterController').cmbWave_no3504change();
		    							}else
		    							{
		    								Ext.getCmp('cmbBatch_no3504').getStore().removeAll();
		    								Ext.getCmp('cmbBatch_no3504').setValue(null);
	    				...
	    				},
	    				{
	    					...
	    					store:Ext.create("cms.store.common.comboStore",
	    					{
	    						...
	    						listeners:{  
	    							'load':function(th,records,successful,eOpts ){
	    								if(th.count()>0){
	    									Ext.getCmp('cmbBatch_no3504').setValue(th.getAt(0).data.value);
	    									_myAppGlobal.getController('cms.controller.odata.odata_B2COutstockLabelWaterController').cmbBatch_no3504change();
	    								}else
	    								{
	    									Ext.getCmp('gridOutstockM3504').getStore().removeAll();
	    							   		Ext.getCmp('gridOutstockDlabel3504').getStore().removeAll();
	    		   ...
	    		   ,{
	    				...
	    				region : 'center',
	    				height:'220',
	    				store:gridOutstockM3504store,
	    		        id: 'gridOutstockM3504',
	    				columns:[{			
	    					xtype : 'rownumberer',
	    					width : 30
	    				},
	    			...
	    			{ 
	    				...
	    				items:[
	    				{
		    				xtype:'grid',
		    				region : 'west',
		    				id: 'gridOutstockDlabel3504',
		    				store:gridOutstockDLabel3504store,
		    				height:250,
		    				width:'15%',
		    				columns:[{			
		    					xtype : 'rownumberer',
		    			...
		    			},
	    			{
	    				xtype:'grid',
	    				region : 'east',
	    				store:gridOutstockD3504store,
	    		        id: 'gridOutstockD3504',
	    				height:250,
						width:'85%',
	    				selType : 'cellmodel',

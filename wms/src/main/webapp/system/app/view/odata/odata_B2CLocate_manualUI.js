		...
		'load':function(th,records,successful,eOpts ){
			...
			if(th.data.length>0){
				...
				/*	if(th.data.items[i].data.status==1){
						Ext.getCmp('grid_01_3211').getSelectionModel().select(i,true);
					}*/
				}
				Ext.getCmp('calculateQty3211').setText(th.data.items[0].data.calculateCount);
				Ext.getCmp('grid_01_3211').getSelectionModel().selectAll();

				
				
			}
			Ext.getCmp('lblexpCount3211').setText(sum);
		}
	}
});
...
Ext.define('cms.view.odata.odata_B2CLocate_manualUI',{
	alias:'widget.odata_B2CLocate_manualUI',
	title:'电商手动出货调度',
	width:'100%',
	layout:'border',
			    ...
			    items :[
	            {
	    			...
	    			store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbOwnerNo3211').getStore().data.length>0)
								{
									Ext.getCmp('cmbOwnerNo3211').setValue(Ext.getCmp('cmbOwnerNo3211').getStore().getAt(0).data.value);		
								}
							}
						}
					})
	    			...
	    			store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('orgNo3211').getStore().data.length>0)
								{
									Ext.getCmp('orgNo3211').setValue(Ext.getCmp('orgNo3211').getStore().getAt(0).data.value);		
								}
							}
						}
					})	            
		        	...
		        	store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbExpType3211').getStore().data.length>0)
								{
									Ext.getCmp('cmbExpType3211').setValue(Ext.getCmp('cmbExpType3211').getStore().getAt(0).data.value);		
								}
							}
						}
					})
		        	...
		        	store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbDeliverType3211').getStore().data.length>0)
								{
									Ext.getCmp('cmbDeliverType3211').setValue(Ext.getCmp('cmbDeliverType3211').getStore().getAt(0).data.value);		
								}
							}
						}
					})
				...
				//	ChangePage: false,
				    ...
				    listeners : {  
		                    "beforechange" : function(bbar, params){  
		                        debugger
		                    //    Ext.getCmp("grid_01_3211").clearManagedListeners( );
		                        Ext.getCmp("grid_01_3211").removeListener("deselect",
		                        		_myAppGlobal.getController('cms.controller.odata.odata_B2CLocate_manualController')
										.grid_01_3211Deselect());
		                    }  
		                }, 
					displayInfo : true
				}]*/
		    },
		    {
		    	...
		    	viewConfig : {   
	                 ...
	                 getRowClass : function(record,rowIndex,rowParams,store){   
	                    //缺量数据显示红色   
	                    if(record.data.noEnoughQty>0 && Ext.getCmp('cbShowDetail3211').getValue()==true){   

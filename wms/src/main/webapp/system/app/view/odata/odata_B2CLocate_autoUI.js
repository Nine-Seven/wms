			...
			if(th.data.length>0){
				for(var i=0;i<th.data.length;i++){
					...
					/*if(th.data.items[i].data.status==1){
						Ext.getCmp('grid_01_3201').getSelectionModel().select(i,true);
					}*/
				}
				Ext.getCmp('calculateQty3201').setText(th.data.items[0].data.calculateCount);
				Ext.getCmp('grid_01_3201').getSelectionModel().selectAll();

			}
			Ext.getCmp('lblexpCount3201').setText(sum);
		}
	}
});
...
Ext.define('cms.view.odata.odata_B2CLocate_autoUI',{
	alias:'widget.odata_B2CLocate_autoUI',
	title:'电商自动出货调度',
	width:'100%',
	layout:'border',
			    ...
			    items :[
	            {
	    			...
	    			store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbOwnerNo3201').getStore().data.length>0)
								{
									Ext.getCmp('cmbOwnerNo3201').setValue(Ext.getCmp('cmbOwnerNo3201').getStore().getAt(0).data.value);		
								}
							}
						}
					})
	    			...
	    			store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('orgNo3201').getStore().data.length>0)
								{
									Ext.getCmp('orgNo3201').setValue(Ext.getCmp('orgNo3201').getStore().getAt(0).data.value);		
								}
							}
						}
					})            
		        	...
		        	store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbExpType3201').getStore().data.length>0)
								{
									Ext.getCmp('cmbExpType3201').setValue(Ext.getCmp('cmbExpType3201').getStore().getAt(0).data.value);		
								}
							}
						}
					})
		        	...
		        	store:Ext.create('cms.store.bdef.bdef_DefOwnerComboStore',{
	    				...
	    				listeners:{
							'load':function(th,records,successful,eOpts )
							{
								if(Ext.getCmp('cmbDeliverType3201').getStore().data.length>0)
								{
									Ext.getCmp('cmbDeliverType3201').setValue(Ext.getCmp('cmbDeliverType3201').getStore().getAt(0).data.value);		
								}
							}
						}
					})
				...
				//	ChangePage: false,
				    ...
				    listeners : {  
		                    "beforechange" : function(bbar, params){  
		                        debugger
		                    //    Ext.getCmp("grid_01_3201").clearManagedListeners( );
		                        Ext.getCmp("grid_01_3201").removeListener("deselect",
		                        		_myAppGlobal.getController('cms.controller.odata.odata_B2CLocate_autoController')
										.grid_01_3201Deselect());
		                    }  
		                }, 
					displayInfo : true
				}]*/
		    },
		    {
		    	...
		    	viewConfig : {   
	                 ...
	                 getRowClass : function(record,rowIndex,rowParams,store){   
	                    //缺量数据显示红色   
	                    if(record.data.noEnoughQty>0 && Ext.getCmp('cbShowDetail3201').getValue()==true){   

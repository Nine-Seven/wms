 ...
 */
var saveFlag9109=0;
Ext.define('cms.controller.report.report_WorkloadController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_WorkloadUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_WorkloadUI button[name=query]':{
				click:this.queryClick
			},
			//刷新
			'report_WorkloadUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_WorkloadUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_WorkloadUI combo[id=cmbReportSelect9109]':{
				select:this.cmbReportSelect9109Select
			},
			//参考项目选择
			'report_WorkloadUI combo[id=cmbProjectSelect9109]':{
				change:this.cmbProjectSelect9109Select
			},
			//查询按扭
			'report_WorkloadUI button[id=btnSearch9109]':{
				click:this.btnSearch9109Click
			},
			//项目设定按扭
			'report_WorkloadUI button[id=btnProjectSet9109]':{
				click:this.btnProjectSet9109Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9109').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9109');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9109').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9109"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9109"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9109').getValue();
		queryGrid='gridReportColumn9109';	
		Ext.getCmp('gridReportColumn9109').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9109').getValue();
		if(Ext.getCmp('cmbProjectSelect9109').getValue()!="" &&Ext.getCmp('cmbProjectSelect9109').getValue()!=null){
			Ext.getCmp('gridReportColumn9109').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9109').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9109').setValue(null);
		Ext.getCmp('cmbProjectSelect9109').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9109');
	},
	//报表获得焦点
	cmbReportSelect9109Focus:function(){
		Ext.getCmp('cmbProjectSelect9109').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9109Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9109').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9109').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9109').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9109').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9109Select:function(combo){
		this.commonReportTitle9109(Ext.getCmp('cmbReportSelect9109').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9109Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9109').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9109').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9109').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9109').getStore().removeAll();
		Ext.getCmp('gridReportColumn9109').getStore().load();
	},
	
	//项目设定
	btnProjectSet9109Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9109');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9109').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9109').getValue();
		var params = {
	...
	commonReportTitle9109:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9109').removeAll();
					Ext.getCmp('panelReportColumn9109').add(grid);  
					Ext.getCmp('panelReportColumn9109').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9109').removeAll();
				}
			}

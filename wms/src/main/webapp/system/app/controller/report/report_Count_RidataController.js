 ...
 */
var saveFlagH103=0;
Ext.define('cms.controller.report.report_Count_RidataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_Count_RidataUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_Count_RidataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_Count_RidataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_Count_RidataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_Count_RidataUI combo[id=cmbReportSelectH103]':{
				select:this.cmbReportSelectH103Select
			},
			//参考项目选择
			'report_Count_RidataUI combo[id=cmbProjectSelectH103]':{
				change:this.cmbProjectSelectH103Select
			},
			//查询按扭
			'report_Count_RidataUI button[id=btnSearchH103]':{
				click:this.btnSearchH103Click
			},
			//项目设定按扭
			'report_Count_RidataUI button[id=btnProjectSetH103]':{
				click:this.btnProjectSetH103Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelectH103').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'H103');  
		var chartIdTest = Ext.getCmp('cmbReportSelectH103').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocalH103"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocalH103"+chartIdTest);	  //7-13
		}else{
		...
		queryModuleId=Ext.getCmp('cmbReportSelectH103').getValue();
		queryGrid='gridReportColumnH103';	
		Ext.getCmp('gridReportColumnH103').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelectH103').getValue();
		
		if(Ext.getCmp('cmbProjectSelectH103').getValue()!=""&&Ext.getCmp('cmbProjectSelectH103').getValue()!=null){
			Ext.getCmp('gridReportColumnH103').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelectH103').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelectH103').setValue(null);
		Ext.getCmp('cmbProjectSelectH103').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumnH103');
	},	
	//报表获得焦点
	cmbReportSelectH103Focus:function(){
		Ext.getCmp('cmbProjectSelectH103').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelectH103Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelectH103').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelectH103').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelectH103').getStore().removeAll();
		Ext.getCmp('cmbProjectSelectH103').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelectH103Select:function(combo){
		this.commonReportTitleH103(Ext.getCmp('cmbReportSelectH103').getValue(),combo.getValue());
	},
	
	//查询
	btnSearchH103Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelectH103').getValue(),
			strName:Ext.getCmp('cmbProjectSelectH103').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumnH103').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumnH103').getStore().removeAll();
		Ext.getCmp('gridReportColumnH103').getStore().load();
	},
	
	//项目设定
	btnProjectSetH103Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('H103');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelectH103').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelectH103').getValue();
		var params = {
	...
	commonReportTitleH103:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumnH103').removeAll();
					Ext.getCmp('panelReportColumnH103').add(grid);  
					Ext.getCmp('panelReportColumnH103').doLayout();
				}else{
					Ext.getCmp('panelReportColumnH103').removeAll();
				}
			}

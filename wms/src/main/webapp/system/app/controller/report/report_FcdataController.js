 ...
 */
var saveFlag9108=0;
Ext.define('cms.controller.report.report_FcdataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_FcdataUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_FcdataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_FcdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_FcdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_FcdataUI combo[id=cmbReportSelect9108]':{
				select:this.cmbReportSelect9108Select
			},
			//参考项目选择
			'report_FcdataUI combo[id=cmbProjectSelect9108]':{
				change:this.cmbProjectSelect9108Select
			},
			//查询按扭
			'report_FcdataUI button[id=btnSearch9108]':{
				click:this.btnSearch9108Click
			},
			//项目设定按扭
			'report_FcdataUI button[id=btnProjectSet9108]':{
				click:this.btnProjectSet9108Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9108').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9108');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9108').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9108"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9108"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9108').getValue();
		queryGrid='gridReportColumn9108';	
		Ext.getCmp('gridReportColumn9108').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9108').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9108').getValue()!=""&&Ext.getCmp('cmbProjectSelect9108').getValue()!=null){
			Ext.getCmp('gridReportColumn9108').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9108').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9108').setValue(null);
		Ext.getCmp('cmbProjectSelect9108').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9108');
	},	
	//报表获得焦点
	cmbReportSelect9108Focus:function(){
		Ext.getCmp('cmbProjectSelect9108').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9108Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9108').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9108').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9108').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9108').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9108Select:function(combo){
		this.commonReportTitle9108(Ext.getCmp('cmbReportSelect9108').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9108Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9108').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9108').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9108').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9108').getStore().removeAll();
		Ext.getCmp('gridReportColumn9108').getStore().load();
	},
	
	//项目设定
	btnProjectSet9108Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9108');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9108').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9108').getValue();
		var params = {
	...
	commonReportTitle9108:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9108').removeAll();
					Ext.getCmp('panelReportColumn9108').add(grid);  
					Ext.getCmp('panelReportColumn9108').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9108').removeAll();
				}
			}

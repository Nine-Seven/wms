 ...
 */
var saveFlag9201=0;
Ext.define('cms.controller.report.report_Work_IdataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_Work_IdataUI'
			 ],
	init:function(){
		this.control({
			//查找
			'report_Work_IdataUI button[name=query]':{
				click:this.queryClick
			},
			//刷新
			'report_Work_IdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_Work_IdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_Work_IdataUI combo[id=cmbReportSelect9201]':{
				select:this.cmbReportSelect9201Select
			},
			//参考项目选择
			'report_Work_IdataUI combo[id=cmbProjectSelect9201]':{
				change:this.cmbProjectSelect9201Select
			},
			//查询按扭
			'report_Work_IdataUI button[id=btnSearch9201]':{
				click:this.btnSearch9201Click
			},
			//项目设定按扭
			'report_Work_IdataUI button[id=btnProjectSet9201]':{
				click:this.btnProjectSet9201Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9201').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9201');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9201').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9201"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9201"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		
		queryModuleId=Ext.getCmp('cmbReportSelect9201').getValue();
		queryGrid='gridReportColumn9201';	
		Ext.getCmp('gridReportColumn9201').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9201').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9201').getValue()!=""&&Ext.getCmp('cmbProjectSelect9201').getValue()!=null){
			Ext.getCmp('gridReportColumn9201').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9201').getValue();
		}
	...
	refresh:function(){
		...
		Ext.getCmp('cmbProjectSelect9201').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9201');
	},	
	//报表获得焦点
	cmbReportSelect9201Focus:function(){
		Ext.getCmp('cmbProjectSelect9201').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9201Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9201').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9201').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9201').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9201').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9201Select:function(combo){
		this.commonReportTitle9201(Ext.getCmp('cmbReportSelect9201').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9201Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9201').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9201').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9201').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9201').getStore().removeAll();
		Ext.getCmp('gridReportColumn9201').getStore().load();
	},
	
	//项目设定
	btnProjectSet9201Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9201');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9201').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9201').getValue();
		var params = {
	...
	commonReportTitle9201:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9201').removeAll();
					Ext.getCmp('panelReportColumn9201').add(grid);  
					Ext.getCmp('panelReportColumn9201').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9201').removeAll();
				}
			}

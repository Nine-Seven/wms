 ...
 */
var saveFlag9203=0;
Ext.define('cms.controller.report.report_Work_RidataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_Work_RidataUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_Work_RidataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_Work_RidataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_Work_RidataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_Work_RidataUI combo[id=cmbReportSelect9203]':{
				select:this.cmbReportSelect9203Select
			},
			//参考项目选择
			'report_Work_RidataUI combo[id=cmbProjectSelect9203]':{
				change:this.cmbProjectSelect9203Select
			},
			//查询按扭
			'report_Work_RidataUI button[id=btnSearch9203]':{
				click:this.btnSearch9203Click
			},
			//项目设定按扭
			'report_Work_RidataUI button[id=btnProjectSet9203]':{
				click:this.btnProjectSet9203Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9203').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9203');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9203').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9203"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9203"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		
		queryModuleId=Ext.getCmp('cmbReportSelect9203').getValue();
		queryGrid='gridReportColumn9203';	
		Ext.getCmp('gridReportColumn9203').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9203').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9203').getValue()!=""&&Ext.getCmp('cmbProjectSelect9203').getValue()!=null){
			Ext.getCmp('gridReportColumn9203').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9203').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9203').setValue(null);
		Ext.getCmp('cmbProjectSelect9203').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9203');
	},	
	//报表获得焦点
	cmbReportSelect9203Focus:function(){
		Ext.getCmp('cmbProjectSelect9203').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9203Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9203').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9203').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9203').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9203').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9203Select:function(combo){
		this.commonReportTitle9203(Ext.getCmp('cmbReportSelect9203').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9203Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9203').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9203').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9203').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9203').getStore().removeAll();
		Ext.getCmp('gridReportColumn9203').getStore().load();
	},
	
	//项目设定
	btnProjectSet9203Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9203');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9203').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9203').getValue();
		var params = {
	...
	commonReportTitle9203:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9203').removeAll();
					Ext.getCmp('panelReportColumn9203').add(grid);  
					Ext.getCmp('panelReportColumn9203').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9203').removeAll();
				}
			}

 ...
 */
var saveFlag9103=0;
Ext.define('cms.controller.report.report_OdataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_OdataUI'
			 ],
	init:function(){
		this.control({
			//查找
			'report_OdataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_OdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_OdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_OdataUI combo[id=cmbReportSelect9103]':{
				select:this.cmbReportSelect9103Select
			},
			//参考项目选择
			'report_OdataUI combo[id=cmbProjectSelect9103]':{
				change:this.cmbProjectSelect9103Select
			},
			//查询按扭
			'report_OdataUI button[id=btnSearch9103]':{
				click:this.btnSearch9103Click
			},
			//项目设定按扭
			'report_OdataUI button[id=btnProjectSet9103]':{
				click:this.btnProjectSet9103Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9103').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9103');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9103').getValue();
		...
		//debugger;
		var mess = Ext.decode(localStorage.getItem("commonLocal9103"+chartIdTest));   //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9103"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9103').getValue();
		queryGrid='gridReportColumn9103';	
		Ext.getCmp('gridReportColumn9103').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9103').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9103').getValue()!=""&&Ext.getCmp('cmbProjectSelect9103').getValue()!=null){
			Ext.getCmp('gridReportColumn9103').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9103').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9103').setValue(null);
		Ext.getCmp('cmbProjectSelect9103').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9103');
	},
	//报表获得焦点
	cmbReportSelect9103Focus:function(){
		Ext.getCmp('cmbProjectSelect9103').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9103Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9103').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9103').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9103').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9103').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9103Select:function(combo){
		this.commonReportTitle9103(Ext.getCmp('cmbReportSelect9103').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9103Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9103').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9103').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9103').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9103').getStore().removeAll();
		Ext.getCmp('gridReportColumn9103').getStore().load();
	},
	
	//项目设定
	btnProjectSet9103Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9103');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9103').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9103').getValue();
		var params = {
	...
	commonReportTitle9103:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9103').removeAll();
					Ext.getCmp('panelReportColumn9103').add(grid);  
					Ext.getCmp('panelReportColumn9103').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9103').removeAll();
				}
			}

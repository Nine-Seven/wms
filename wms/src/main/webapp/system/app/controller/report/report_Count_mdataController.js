 ...
 */
var saveFlagH105=0;
Ext.define('cms.controller.report.report_Count_mdataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_Count_mdataUI'
			 ],
	init:function(){
		this.control({
			//查找
			'report_Count_mdataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_Count_mdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_Count_mdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_Count_mdataUI combo[id=cmbReportSelectH105]':{
				select:this.cmbReportSelectH105Select
			},
			//参考项目选择
			'report_Count_mdataUI combo[id=cmbProjectSelectH105]':{
				change:this.cmbProjectSelectH105Select
			},
			//查询按扭
			'report_Count_mdataUI button[id=btnSearchH105]':{
				click:this.btnSearchH105Click
			},
			//项目设定按扭
			'report_Count_mdataUI button[id=btnProjectSetH105]':{
				click:this.btnProjectSetH105Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelectH105').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'H105');  
		var chartIdTest = Ext.getCmp('cmbReportSelectH105').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocalH105"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocalH105"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		
		
		queryModuleId=Ext.getCmp('cmbReportSelectH105').getValue();
		queryGrid='gridReportColumnH105';	
		Ext.getCmp('gridReportColumnH105').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelectH105').getValue();
		
		if(Ext.getCmp('cmbProjectSelectH105').getValue()!=""&&Ext.getCmp('cmbProjectSelectH105').getValue()!=null){
			Ext.getCmp('gridReportColumnH105').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelectH105').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelectH105').setValue(null);
		Ext.getCmp('cmbProjectSelectH105').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumnH105');
	},
	//报表获得焦点
	cmbReportSelectH105Focus:function(){
		Ext.getCmp('cmbProjectSelectH105').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelectH105Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelectH105').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelectH105').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelectH105').getStore().removeAll();
		Ext.getCmp('cmbProjectSelectH105').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelectH105Select:function(combo){
		this.commonReportTitleH105(Ext.getCmp('cmbReportSelectH105').getValue(),combo.getValue());
	},
	
	//查询
	btnSearchH105Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelectH105').getValue(),
			strName:Ext.getCmp('cmbProjectSelectH105').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumnH105').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumnH105').getStore().removeAll();
		Ext.getCmp('gridReportColumnH105').getStore().load();
	},
	
	//项目设定
	btnProjectSetH105Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('H105');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelectH105').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelectH105').getValue();
		var params = {
	...
	commonReportTitleH105:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumnH105').removeAll();
					Ext.getCmp('panelReportColumnH105').add(grid);  
					Ext.getCmp('panelReportColumnH105').doLayout();
				}else{
					Ext.getCmp('panelReportColumnH105').removeAll();
				}
			}

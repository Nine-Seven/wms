 ...
 */
var saveFlag9102=0;
Ext.define('cms.controller.report.report_RidataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_RidataUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_RidataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_RidataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_RidataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_RidataUI combo[id=cmbReportSelect9102]':{
				select:this.cmbReportSelect9102Select
			},
			//参考项目选择
			'report_RidataUI combo[id=cmbProjectSelect9102]':{
				change:this.cmbProjectSelect9102Select
			},
			//查询按扭
			'report_RidataUI button[id=btnSearch9102]':{
				click:this.btnSearch9102Click
			},
			//项目设定按扭
			'report_RidataUI button[id=btnProjectSet9102]':{
				click:this.btnProjectSet9102Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9102').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9102');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9102').getValue();
		...
		//debugger;
		var mess = Ext.decode(localStorage.getItem("commonLocal9102"+chartIdTest));   //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9102"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9102').getValue();
		queryGrid='gridReportColumn9102';	
		Ext.getCmp('gridReportColumn9102').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9102').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9102').getValue()!=""&&Ext.getCmp('cmbProjectSelect9102').getValue()!=null){
			Ext.getCmp('gridReportColumn9102').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9102').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9102').setValue(null);
		Ext.getCmp('cmbProjectSelect9102').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9102');
	},	
	//报表获得焦点
	cmbReportSelect9102Focus:function(){
		Ext.getCmp('cmbProjectSelect9102').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9102Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9102').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9102').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9102').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9102').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9102Select:function(combo){
		this.commonReportTitle9102(Ext.getCmp('cmbReportSelect9102').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9102Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9102').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9102').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9102').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9102').getStore().removeAll();
		Ext.getCmp('gridReportColumn9102').getStore().load();
	},
	
	//项目设定
	btnProjectSet9102Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9102');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9102').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9102').getValue();
		var params = {
	...
	commonReportTitle9102:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9102').removeAll();
					Ext.getCmp('panelReportColumn9102').add(grid);  
					Ext.getCmp('panelReportColumn9102').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9102').removeAll();
				}
			}

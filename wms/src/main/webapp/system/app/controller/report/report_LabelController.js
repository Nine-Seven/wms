 ...
 */
var saveFlag9104=0;
Ext.define('cms.controller.report.report_LabelController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_LabelUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_LabelUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_LabelUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_LabelUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_LabelUI combo[id=cmbReportSelect9104]':{
				select:this.cmbReportSelect9104Select
			},
			//参考项目选择
			'report_LabelUI combo[id=cmbProjectSelect9104]':{
				change:this.cmbProjectSelect9104Select
			},
			//查询按扭
			'report_LabelUI button[id=btnSearch9104]':{
				click:this.btnSearch9104Click
			},
			//项目设定按扭
			'report_LabelUI button[id=btnProjectSet9104]':{
				click:this.btnProjectSet9104Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9104').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9104');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9104').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9104"+chartIdTest));   //7-13
		...
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9104"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		
		queryModuleId=Ext.getCmp('cmbReportSelect9104').getValue();
		queryGrid='gridReportColumn9104';	
		Ext.getCmp('gridReportColumn9104').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9104').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9104').getValue()!=""&&Ext.getCmp('cmbProjectSelect9104').getValue()!=null){
			Ext.getCmp('gridReportColumn9104').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9104').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9104').setValue(null);
		Ext.getCmp('cmbProjectSelect9104').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9104');
	},
	//报表获得焦点
	cmbReportSelect9104Focus:function(){
		Ext.getCmp('cmbProjectSelect9104').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9104Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9104').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9104').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9104').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9104').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9104Select:function(combo){
		this.commonReportTitle9104(Ext.getCmp('cmbReportSelect9104').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9104Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9104').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9104').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9104').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9104').getStore().removeAll();
		Ext.getCmp('gridReportColumn9104').getStore().load();
	},
	
	//项目设定
	btnProjectSet9104Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9104');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9104').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9104').getValue();
		var params = {
	...
	commonReportTitle9104:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9104').removeAll();
					Ext.getCmp('panelReportColumn9104').add(grid);  
					Ext.getCmp('panelReportColumn9104').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9104').removeAll();
				}
			}

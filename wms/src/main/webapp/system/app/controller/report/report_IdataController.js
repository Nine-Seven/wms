 ...
 */
var saveFlag9106=0;
Ext.define('cms.controller.report.report_IdataController',{
	...
	requires:[
			 ...
			 'cms.view.report.report_IdataUI'
			 ],
	init:function(){
		this.control({
			//查找
			'report_IdataUI button[name=query]':{
				click:this.queryClick
			},
			//刷新
			'report_IdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_IdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_IdataUI combo[id=cmbReportSelect9106]':{
				select:this.cmbReportSelect9106Select
			},
			//参考项目选择
			'report_IdataUI combo[id=cmbProjectSelect9106]':{
				change:this.cmbProjectSelect9106Select
			},
			//查询按扭
			'report_IdataUI button[id=btnSearch9106]':{
				click:this.btnSearch9106Click
			},
			//项目设定按扭
			'report_IdataUI button[id=btnProjectSet9106]':{
				click:this.btnProjectSet9106Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9106').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9106');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9106').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9106"+chartIdTest));   //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9106"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9106').getValue();
		queryGrid='gridReportColumn9106';	
		Ext.getCmp('gridReportColumn9106').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9106').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9106').getValue()!=""&&Ext.getCmp('cmbProjectSelect9106').getValue()!=null){
			Ext.getCmp('gridReportColumn9106').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9106').getValue();
		}	
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9106').setValue(null);
		Ext.getCmp('cmbProjectSelect9106').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9106');
	},	
	//报表获得焦点
	cmbReportSelect9106Focus:function(){
		Ext.getCmp('cmbProjectSelect9106').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9106Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9106').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9106').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9106').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9106').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9106Select:function(combo){
		this.commonReportTitle9106(Ext.getCmp('cmbReportSelect9106').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9106Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9106').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9106').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9106').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9106').getStore().removeAll();
		Ext.getCmp('gridReportColumn9106').getStore().load();
	},
	
	//项目设定
	btnProjectSet9106Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9106');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9106').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9106').getValue();
		var params = {
	...
	commonReportTitle9106:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9106').removeAll();
					Ext.getCmp('panelReportColumn9106').add(grid);  
					Ext.getCmp('panelReportColumn9106').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9106').removeAll();
				}
			}

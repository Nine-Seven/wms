 ...
 */
var saveFlag9205=0;
Ext.define('cms.controller.report.report_FcdataWorkController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_WorkFcdataUI'
			 ],	
	init:function(){
		this.control({
			//查找
			'report_WorkFcdataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_WorkFcdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_WorkFcdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_WorkFcdataUI combo[id=cmbReportSelect9205]':{
				select:this.cmbReportSelect9205Select
			},
			//参考项目选择
			'report_WorkFcdataUI combo[id=cmbProjectSelect9205]':{
				change:this.cmbProjectSelect9205Select
			},
			//查询按扭
			'report_WorkFcdataUI button[id=btnSearch9205]':{
				click:this.btnSearch9205Click
			},
			//项目设定按扭
			'report_WorkFcdataUI button[id=btnProjectSet9205]':{
				click:this.btnProjectSet9205Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9205').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9205');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9205').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9205"+chartIdTest)); //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9205"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		queryModuleId=Ext.getCmp('cmbReportSelect9205').getValue();
		queryGrid='gridReportColumn9205';	
		Ext.getCmp('gridReportColumn9205').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9205').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9205').getValue()!=""&&Ext.getCmp('cmbProjectSelect9205').getValue()!=null){
			Ext.getCmp('gridReportColumn9205').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9205').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9205').setValue(null);
		Ext.getCmp('cmbProjectSelect9205').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9205');
	},	
	//报表获得焦点
	cmbReportSelect9205Focus:function(){
		Ext.getCmp('cmbProjectSelect9205').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9205Select:function(combo){
		//清空
		Ext.getCmp('cmbProjectSelect9205').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9205').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9205').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9205').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9205Select:function(combo){
		this.commonReportTitle9205(Ext.getCmp('cmbReportSelect9205').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9205Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9205').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9205').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9205').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9205').getStore().removeAll();
		Ext.getCmp('gridReportColumn9205').getStore().load();
	},
	
	//项目设定
	btnProjectSet9205Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9205');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9205').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9205').getValue();
		var params = {
	...
	commonReportTitle9205:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9205').removeAll();
					Ext.getCmp('panelReportColumn9205').add(grid);  
					Ext.getCmp('panelReportColumn9205').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9205').removeAll();
				}
			}

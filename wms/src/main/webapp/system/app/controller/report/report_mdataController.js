 ...
 */
var saveFlag9114=0;
Ext.define('cms.controller.report.report_mdataController',{
	extend:'Ext.app.Controller',
	requires:[
			 'cms.view.report.window.reportCheckSetWindow',
			 'cms.view.report.window.renameWindow',
			 'cms.view.report.report_mdataUI'
			 ],
	init:function(){
		this.control({
			//查找
			'report_mdataUI button[name=query]':{
				click:this.queryClick
			},
			//查找
			'report_mdataUI button[name=refresh]':{
				click:this.refresh
			},
			//导出
			'report_mdataUI button[name=export]':{
				click:this.exportClick
			},			
			//报表选择
			'report_mdataUI combo[id=cmbReportSelect9114]':{
				select:this.cmbReportSelect9114Select
			},
			//参考项目选择
			'report_mdataUI combo[id=cmbProjectSelect9114]':{
				change:this.cmbProjectSelect9114Select
			},
			//查询按扭
			'report_mdataUI button[id=btnSearch9114]':{
				click:this.btnSearch9114Click
			},
			//项目设定按扭
			'report_mdataUI button[id=btnProjectSet9114]':{
				click:this.btnProjectSet9114Click
			}
		});
	},
	...
	//查找
	queryClick:function(){
		if(Ext.isEmpty(Ext.getCmp('cmbReportSelect9114').getValue())){
			Ext.example.msg($i18n.prompt,'请先选择一份报表');
			return;
		}
		
		//保存模块id和报表id，用于区分查询条件   7-13
		localStorage.setItem("moduleId",'9114');  
		var chartIdTest = Ext.getCmp('cmbReportSelect9114').getValue();
		...
		var mess = Ext.decode(localStorage.getItem("commonLocal9114"+chartIdTest));   //7-13
		if(mess!=null){
			...
			localStorage.removeItem("commonLocal9114"+chartIdTest);	  //7-13
		}else{
			getQueryWindow.add(Ext.create('cms.view.common.queryPanel'));
		}
		
		queryModuleId=Ext.getCmp('cmbReportSelect9114').getValue();
		queryGrid='gridReportColumn9114';	
		Ext.getCmp('gridReportColumn9114').getStore().proxy.extraParams.moduleId = Ext.getCmp('cmbReportSelect9114').getValue();
		
		if(Ext.getCmp('cmbProjectSelect9114').getValue()!=""&&Ext.getCmp('cmbProjectSelect9114').getValue()!=null){
			Ext.getCmp('gridReportColumn9114').getStore().proxy.extraParams.strName = Ext.getCmp('cmbProjectSelect9114').getValue();
		}
	},
	//刷新
	refresh:function(){
		Ext.getCmp('cmbProjectSelect9114').setValue(null);
		Ext.getCmp('cmbProjectSelect9114').getStore().reload();
	},
	exportClick:function()
	{
		commExport('gridReportColumn9114');
	},
	//报表获得焦点
	cmbReportSelect9114Focus:function(){
		Ext.getCmp('cmbProjectSelect9114').setValue(null);
	},
	
	//报表选择事件
	cmbReportSelect9114Select:function(combo){
		debugger;
		//清空
		Ext.getCmp('cmbProjectSelect9114').setValue(null);
		...
		Ext.apply(Ext.getCmp('cmbProjectSelect9114').getStore().proxy.extraParams,strWheresql);
		Ext.getCmp('cmbProjectSelect9114').getStore().removeAll();
		Ext.getCmp('cmbProjectSelect9114').getStore().load();
	},
	
	//参考项目选择事件
	cmbProjectSelect9114Select:function(combo){
		this.commonReportTitle9114(Ext.getCmp('cmbReportSelect9114').getValue(),combo.getValue());
	},
	
	//查询
	btnSearch9114Click:function(){
		var params = {
			moduleId:Ext.getCmp('cmbReportSelect9114').getValue(),
			strName:Ext.getCmp('cmbProjectSelect9114').getValue()
		};
		Ext.apply(Ext.getCmp('gridReportColumn9114').getStore().proxy.extraParams,params);
		Ext.getCmp('gridReportColumn9114').getStore().removeAll();
		Ext.getCmp('gridReportColumn9114').getStore().load();
	},
	
	//项目设定
	btnProjectSet9114Click:function(){
		Ext.create('cms.view.report.window.reportCheckSetWindow',{
			title:$i18n.checkSet//高阶查询设置
		}).show();
		Ext.getCmp('moduleId').setValue('9114');
		var cmbReportSelectId = Ext.getCmp('cmbReportSelect9114').getValue();
		var cmbProjectSelectId = Ext.getCmp('cmbProjectSelect9114').getValue();
		var params = {
	...
	commonReportTitle9114:function(strReportId,strProjectId)
	{
		Ext.Ajax.request({
			...
			success : function(response, options) {  
				...
				if(data.isSucc){
	        		...
	        		Ext.getCmp('panelReportColumn9114').removeAll();
					Ext.getCmp('panelReportColumn9114').add(grid);  
					Ext.getCmp('panelReportColumn9114').doLayout();
				}else{
					Ext.getCmp('panelReportColumn9114').removeAll();
				}
			}
